...
- [新功能] GitHub Actions 的 `daily_analysis.yml` 与 `deep_analysis.yml` 新增 `analysis-db` Artifact 上传，可下载云端 `stock_analysis.db` 快照。
- [改进] `main.py` / `webui.py` 启动时新增云端历史自动同步：拉取 `analysis-db` 并合并到本地 `analysis_history` / `news_intel` / `fundamental_snapshot`。
- [改进] Cloud history 同步日志新增 `codes` 字段，启动后可直接看到本次合并涉及的股票代码列表。
- [改进] Cloud history 合并策略调整为按 `(query_id, code, created_at)` 时间覆盖：同一 query/股票若云端记录更晚则替换本地旧记录，避免 WebUI 显示过期时间。
